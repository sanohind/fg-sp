...
<div class="p-6">
    ...
    <div class="mb-6">
        <nav class="text-sm font-semibold mb-2" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li class="flex items-center">
                    <span class="text-[#0A2856]">Slot History</span>
                </li>
            </ol>
        </nav>
        <h1 class="text-2xl font-bold text-gray-900">Slot History</h1>
    </div>

    <!-- Slot Info Card -->
    <div class="bg-white p-4 rounded-lg shadow-sm border mb-8">
        <h2 class="text-xl font-semibold text-gray-700 mb-2">Slot: {{ $slot->slot_name }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <span class="text-gray-600">Rack:</span>
                <span class="font-medium">{{ $slot->rack->rack_name ?? 'N/A' }}</span>
            </div>
            <div>
                <span class="text-gray-600">Capacity:</span>
                <span class="font-medium">{{ $slot->capacity }}</span>
            </div>
            <div>
                <span class="text-gray-600">Current Quantity:</span>
                <span class="font-medium">{{ $slot->current_qty ?? 0 }}</span>
            </div>
        </div>
        @if($slot->item)
        <div class="mt-3 pt-3 border-t">
            <span class="text-gray-600">Assigned Part:</span>
            <span class="font-medium">{{ $slot->item->part_no }} - {{ $slot->item->description }}</span>
        </div>
        @endif
    </div>
    ...
    <div class="overflow-x-auto">
        <table id="slotHistoryTable" class="min-w-full divide-y divide-gray-200">
